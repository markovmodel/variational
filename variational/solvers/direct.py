...
import numpy as np
import scipy.linalg
import variational.solvers.qr_c.qr_solver as vqr
...
def eig_corr_qr(C0, Ct, epsilon=1e-10):
    r""" Solve generalized eigenvalue problem with correlation matrices C0 and Ct
    THIS FUNCTION USES OUR OWN IMPLEMENTATION OF THE QR-ALGORTIHM.
    ...
    # solve the symmetric eigenvalue problem in the new basis
    (l, R_trans) = vqr.qr_solve(Ct_trans)
    (l, R_trans) = sort_by_norm(l, R_trans)
